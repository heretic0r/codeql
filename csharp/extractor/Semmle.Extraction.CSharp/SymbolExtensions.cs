...
namespace Semmle.Extraction.CSharp
{
    ...
    static class SymbolExtensions
    {
        ...
        /// <summary>
        /// Holds if the label generated for `dependant` will contain a reference to
        /// the ID for `symbol`. If this is the case, then the label for `symbol`
        /// must not contain a reference back to `dependant`.
        /// </summary>
        public static bool LabelDependsOn(this ITypeSymbol dependant, Context cx, ISymbol symbol)
        {
            var seen = new HashSet<ITypeSymbol>(SymbolEqualityComparer.Default);

            bool LabelDependsOnImpl(ITypeSymbol type)
            {
                ...
                using (cx.StackGuard)
                {
                    switch (type.TypeKind)
                    {
                        case TypeKind.Array:
                            ...
                            return LabelDependsOnImpl(array.ElementType);
                        case TypeKind.Class:
                        ...
                        case TypeKind.Error:
                            ...
                            if (LabelDependsOnImpl(named.ContainingType))
                                return true;
                            if (LabelDependsOnImpl(named.GetNonObjectBaseType(cx)))
                                return true;
                            if (LabelDependsOnImpl(named.ConstructedFrom))
                                return true;
                            return named.TypeArguments.Any(LabelDependsOnImpl);
                        case TypeKind.Pointer:
                            var ptr = (IPointerTypeSymbol)type;
                            return LabelDependsOnImpl(ptr.PointedAtType);
            ...
            return LabelDependsOnImpl(dependant);
        }
        ...
        /// <summary>
        /// Adds an appropriate label to the trap builder <paramref name="trapFile"/>
        /// for the symbol <paramref name="symbol"/> belonging to
        /// <paramref name="symbolBeingDefined"/>.
        ///
        /// This will either write the ID of the entity belonging to <paramref name="symbol"/>,
        /// or if that will lead to cyclic labels, it will generate an appropriate label that
        /// encodes the signature of <paramref name="symbol" />.
        /// </summary>
        public static void BuildOrWriteId(this ISymbol symbol, Context cx, TextWriter trapFile, ISymbol symbolBeingDefined)
        {
            ...
            else if (symbol is ITypeSymbol type && type.LabelDependsOn(cx, symbolBeingDefined))
        ...
        static void BuildNamedTypeId(this INamedTypeSymbol named, Context cx, TextWriter trapFile, bool addBaseClass
        , ISymbol symbolBeingDefined)
        {
            if (named.IsTupleType)
            {
                trapFile.Write('(');
